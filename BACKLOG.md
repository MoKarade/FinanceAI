...
- [x] 🟠 **`[AI-STOPREASON-JETE]`** ✅ **LIVRÉ au lot 213 (2026-09-07)** — `VisionTronqueeError` levée sur `stop_reason === 'max_tokens'` aux deux appels Vision, cause `tronque` dans `messageErreurIa` (« trop long … réimporte en plusieurs parties ») ; `claude.visionTronquee.test.ts` 5 cas sur le vrai module (SDK simulé) ; perturbation → 1 rouge → à déménager vers BACKLOG_ARCHIVE à la prochaine PR. Contexte d'origine : (S, ÉLEVÉ — sans décision) — `services/claude.ts` ne lit `stop_reason` nulle part
