...
export const Planning: React.FC<PlanningProps> = ({ transactions, savingsGoals = [], setSavingsGoals, budgetItems, setBudgetItems, config, apiKey }) => {
    ...
    // --- 1. RECURRING EXPENSES DETECTOR (Merged Logic) ---
    const heuristicSubs = useMemo(() => {
        const groups: Record<string, Transaction[]> = {};
        const sixMonthsAgo = new Date();
        sixMonthsAgo.setMonth(sixMonthsAgo.getMonth() - 6);

        // EXCLUSION LIST (Strict Mode)
        const variableKeywords = [
            'iga', 'metro', 'provigo', 'super c', 'maxi', 'walmart', 'costco', 'dollama',
            'mcdonald', 'tim horton', 'starbucks', 'uber', 'taxi', 'restaurant', 'resto', 'bar', 'saq',
            'petro', 'shell', 'esso', 'ultramar', 'sonic', 'essence', 'gaz',
            'pharmaprix', 'jean coutu', 'uniprix', 'familiprix',
            'amazon', 'paypal', 'home depot', 'canadian tire'
        ];

        // Group by normalized payee
        transactions
            .filter(t => t.amount < 0 && !t.isTransfer && !t.isDuplicate && new Date(t.date) > sixMonthsAgo)
            .forEach(t => {
                const key = (t.payee || 'Inconnu').toLowerCase().trim()
                    .replace(/[0-9*#]/g, '')
                    .replace('payment', '')
                    .replace('paiement', '')
                    .replace('prel', '')
        ...
        Object.entries(groups).forEach(([key, txs]) => {
            const isKnownService = subKeywords.some(k => key.includes(k));

            // MUST have at least 2 occurrences in 6 months to be considered recurring, NO EXCEPTIONS
            if (txs.length >= 2) {
                ...
                // Stability Check: Is amount consistent? (Subscription prices are usually exact, max $5 diff)
                const isStableAmount = amounts.every(a => Math.abs(a - avg) < 5);

                // Interval Check: Gap between the last two should be ~1 month (20 to 40 days) or ~1 year
                ...
                if (diffDays >= 20 && diffDays <= 40) isValidInterval = true; // Monthly
                if (diffDays >= 350 && diffDays <= 380) isValidInterval = true; // Yearly

                // Strict validation: Must be stable AND have a valid recurring interval
                if (isStableAmount && isValidInterval) {
                    const lastTx = txs[txs.length - 1];
                    const day = new Date(lastTx.date).getDate();
                    ...
                    detected.push({
                        ...
                        yearlyCost: avg * (diffDays >= 350 ? 1 : 12)
                    });
                }
            }
        });
    ...
    // Active List Selection (AI vs Heuristic)
    const activeSubs = aiSubs || heuristicSubs;

    const { totalMonthly, totalYearly, potentialSavings } = useMemo(() => {
        const mTotal = activeSubs.reduce((acc, i) => acc + i.averageAmount, 0);
        const yTotal = mTotal * 12;
        const savings = yTotal * 10 * 1.4; // 10 years at 7% compound
        return { totalMonthly: mTotal, totalYearly: yTotal, potentialSavings: savings };
    }, [activeSubs]);
    ...
    return (
        <div className="space-y-6 animate-fade-in pb-20">
            ...
            <div className="flex flex-col md:flex-row justify-between items-end gap-4 bg-gradient-to-r from-blue-900/20 to-purple-900/20 p-6 rounded-2xl border border-white/10">
                ...
                <div className="flex gap-4">
                    <div className="text-right">
                        <div className="text-[10px] uppercase text-gray-500 font-bold">Fixe Mensuel</div>
                        <div className="text-2xl font-bold text-red-400 privacy-blur">{totalMonthly.toFixed(0)} $</div>
                    </div>
                    <div className="w-px bg-white/10"></div>
                    <div className="text-right">
                        <div className="text-[10px] uppercase text-gray-500 font-bold">Coût Annuel</div>
                        <div className="text-2xl font-bold text-white privacy-blur">{totalYearly.toLocaleString()} $</div>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                ...
                {/* COLUMN 1: SUBSCRIPTIONS & LIST */}
                <div className="xl:col-span-1 space-y-6">
                    <Card title="Abonnements & Récurrents" action={
                        <div className="flex gap-2">
                            {!aiSubs ? (
                                <button onClick={handleAiAnalysis} disabled={isAnalyzing} className="text-[10px] bg-gradient-to-r from-secondary to-purple-600 px-2 py-1 rounded text-white font-bold hover:brightness-110 disabled:opacity-50">
                                    {isAnalyzing ? '...' : '⚡ IA'}
                                </button>
                            ) : (
                                <button onClick={() => setAiSubs(null)} className="text-[10px] bg-white/10 px-2 py-1 rounded text-gray-400">Reset</button>
                            )}
                        </div>
                    }>
                            ...
                            {activeSubs.map((sub, idx) => (
                                <div key={idx} className="flex justify-between items-center p-3 bg-[#1a1a1a] rounded-xl border border-white/5 hover:border-white/20 transition-all group">
                                    <div className="flex items-center gap-3 overflow-hidden">
                                        <div className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-sm shadow-inner flex-shrink-0">
                                            {(sub.payee || '').toLowerCase().includes('netflix') ? '🍿' :
                                                (sub.payee || '').toLowerCase().includes('spotify') ? '🎵' :
                                                    (sub.payee || '').toLowerCase().includes('hydro') ? '⚡' :
                                                        (sub.payee || '').toLowerCase().includes('internet') ? '🌐' :
                                                            (sub.payee || '').toLowerCase().includes('loyer') ? '🏠' : '💳'}
                                        </div>
                                        <div className="min-w-0">
                                            <div className="font-bold text-white text-sm truncate">{sub.payee}</div>
                                            <div className="text-[10px] text-gray-500">Le {sub.dayOfMonth} du mois</div>
                                        </div>
                                    </div>
                                    <div className="text-right flex-shrink-0">
                                        <div className="font-bold text-white privacy-blur">{sub.averageAmount.toFixed(0)}$</div>
                                        <div className="text-[9px] text-gray-600">/mois</div>
                                    </div>
                                </div>
                            ))}
                        ...
                        <div className="mt-4 bg-gradient-to-br from-red-900/20 to-black border border-red-500/20 p-3 rounded-xl flex items-center justify-between">
                            <div>
                                <div className="text-[10px] text-red-300 uppercase font-bold">Le "Latte Factor"</div>
                                <div className="text-[9px] text-gray-400">Si investi à 7% sur 10 ans</div>
                            </div>
                    ...
                    <Card title="Calendrier des Factures">
                        <div className="flex justify-between items-center mb-4 bg-white/5 p-2 rounded-lg">
                            ...
                            <h2 className="text-sm font-bold text-white capitalize">
                                {currentDate.toLocaleString('fr-CA', { month: 'long', year: 'numeric' })}
                            </h2>
                            <button onClick={() => changeMonth(1)} className="p-1 px-3 hover:bg-white/10 rounded text-gray-400">▶</button>
                        </div>
                        ...
                        <div className="grid grid-cols-7 gap-1">
                            {['L', 'M', 'M', 'J', 'V', 'S', 'D'].map(d => (
                                <div key={d} className="text-center text-gray-600 text-[10px] font-bold uppercase pb-1">{d}</div>
                            ))}
                                    ...
                                    <div key={idx} className={`aspect-square rounded-lg border flex flex-col items-center justify-center relative ${isToday ? 'bg-primary/20 border-primary' : hasBills ? 'bg-red-500/10 border-red-500/30' : 'bg-dark/40 border-white/5'}`}>
                                        ...
                                        {hasBills && (
                                            <div className="mt-1 text-center">
                                                <div className="text-[9px] font-bold text-white leading-none">{dailyTotal.toFixed(0)}$</div>
                                                <div className="flex gap-0.5 justify-center mt-0.5">
                                                    {bills.slice(0, 3).map((b, bi) => (
                                                        <div key={bi} className="w-1 h-1 rounded-full bg-red-400" title={b.payee}></div>
                                                    ))}
                                                </div>
                                            </div>
                                        )}
                ...
                <div className="xl:col-span-1 space-y-6">
                    {/* SAVINGS GOALS */}
                    <Card title="Objectifs (Sinking Funds)" action={
                        <button onClick={() => setIsAddingGoal(!isAddingGoal)} className="text-[10px] bg-white/10 hover:bg-white/20 px-2 py-1 rounded text-white">+ Nouveau</button>
                    }>
                        {isAddingGoal && (
                                ...
                                return (
                                    <div key={goal.id} className="relative p-3 bg-[#1a1a1a] rounded-xl border border-white/5 group">
                                        ...
                                        <div className="w-full bg-black/50 rounded-full h-1.5">
                                            <div className="h-full bg-gradient-to-r from-blue-500 to-purple-500 rounded-full" style={{ width: `${Math.min(100, progress)}%` }}></div>
                                        </div>
                                    </div>
                                )
                            })}
