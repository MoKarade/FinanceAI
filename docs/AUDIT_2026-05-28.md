...
- ⏳ perf: `monteCarlo.allRuns` conserve `chartData` complet (scalaires suffisent) ; `ExpertTooltip`/handlers
  `React.memo`/`useMemo` — **reste à faire** (à évaluer : changer ce que stocke `allRuns` touche des consumers).
