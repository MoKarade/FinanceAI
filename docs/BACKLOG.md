...
- [ ] **[A11Y-CHARTS]** 🔧 HIGH (PHASE 1 faite 2026-06-16, phase 2 restante) — graphes Recharts sans alternative textuelle (WCAG 1.1.1 A). **Phase 1 LIVRÉE** : primitif réutilisable `<ChartDataTable>` (table sr-only, caption + en-têtes scope + échantillonnage uniforme ≤40 lignes, formateurs délégués + mode privé) ; intégré dans le wrapper partagé `ZoomableTimeChart` (couvre `StockChart` + `DashboardEvolutionChart`) — placé en FRÈRE du conteneur `role="img"` (un SR ne traverse pas dedans). 3 tests. **Phase 2 — LOTS 1+2 LIVRÉS (2026-06-17)** : LOT 1 = `FutureProjection`/`Retirement` accumulation/`DebtManager` ; LOT 2 = `RealEstate` (scénarios comparatifs — l'amortissement est DÉJÀ un `<table>` HTML accessible), `Investments` (2 donuts géo+secteur), `Budget` (donut 50/30/20), `ChildPlanning` (coût/âge + REEE). Tous → `<ChartDataTable>` sr-only + `role="img"` + **masquage privacy-aware** ($ → `Montant masqué` en mode discret ; les colonnes `%` restent visibles). **RESTE (lot 3)** : `LifeEvents`, `DividendPanel`, `MultiPropertyComparison`, `FutureDetailModal`, + `Retirement` cashflow (2ᵉ graphe).
