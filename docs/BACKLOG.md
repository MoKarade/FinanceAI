...
- [ ] **[FISC-WELCOME-UNIFY]** 🧭 HIGH (RÉEL, confirmé) — taxe de bienvenue : `helpers.ts:80` (MTL 4%) ≠ `realEstate.ts:88` (provincial 2%). L'UI affiche ≠ ce que le moteur ponctionne. **Décision Marc requise** : `RealEstateGoal` n'a pas de champ municipalité → soit défaut MTL, soit ajouter un champ `city` (+ UI) + transcrire les 2 barèmes (MTL + provincial réindexé 2026) dans FISCAL_REFERENCE. Puis unifier 1 fonction consommée par moteur ET UI.
