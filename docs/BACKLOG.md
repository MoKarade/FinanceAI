...
- [ ] **[A11Y-CHARTS]** 🔧 HIGH (PHASE 1 faite 2026-06-16, phase 2 restante) — graphes Recharts sans alternative textuelle (WCAG 1.1.1 A). **Phase 1 LIVRÉE** : primitif réutilisable `<ChartDataTable>` (table sr-only, caption + en-têtes scope + échantillonnage uniforme ≤40 lignes, formateurs délégués + mode privé) ; intégré dans le wrapper partagé `ZoomableTimeChart` (couvre `StockChart` + `DashboardEvolutionChart`) — placé en FRÈRE du conteneur `role="img"` (un SR ne traverse pas dedans). 3 tests. **Phase 2 — LOT 1 LIVRÉ (2026-06-17)** : `FutureProjection` (courbe patrimoine), `Retirement` (accumulation), `DebtManager` (extinction) → `<ChartDataTable>` sr-only + `role="img"` + **masquage privacy-aware** (colonnes $ → `Montant masqué` en mode discret, garde-test). **RESTE (~9, par lots de 3-4)** : RealEstate amortissement, Investments allocation, Budget, ChildPlanning, LifeEvents, DividendPanel, MultiPropertyComparison, FutureDetailModal, + Retirement cashflow (2ᵉ graphe).
