...
## Archivé le 2026-08-17 — rattrapage de tenue (57 items cochés restés dans le BACKLOG)

> ⚠️ **Pourquoi ce lot arrive d'un coup, alors que la règle dit « au plus tard à la PR
> suivante ».** Marc l'a relevé : l'archive n'avait pas bougé depuis le 2026-08-14 et 57 items
> cochés dormaient encore dans `BACKLOG.md`. La cause : je cochais bien au merge, mais je ne
> DÉMÉNAGEAIS plus. C'est le pire des deux mondes — cocher donne au fichier l'apparence d'être
> tenu alors qu'il ne l'est pas, et le PM comme la reprise de session lisent un backlog qui
> mélange le fait et l'à-faire (classe `PM-STALE-BACKLOG`). Le déménagement fait désormais
> partie du commit de livraison, pas d'un ménage ultérieur.
