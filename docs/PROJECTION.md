...
> ⚠️ `estateNetWorth` n'est PAS trié qu'ici. `compareLifeScenarios` l'affiche, et
> `strategyConfigRanking.ts` (recherche de stratégies Monte Carlo) score l'impôt à vie via
> `lifetimeTaxTotal`, qui additionne `totalEstateTax`. Tout correctif qui déplace l'un de ces
> champs doit donc mesurer le CLASSEMENT, pas seulement le montant
...
> (`[DETTE-DEPRECATED-DRAWDOWN]`) — le point d'entrée est `compareLifeScenarios`. Le trieur
> DÉTERMINISTE `rankStrategies` (`strategyRanking.ts`) a été RETIRÉ le 2026-09-05
> (`[ENG-RANKING-MODULES-ORPHELINS]`, décision Marc : aucun appelant ; son score `balanced`
> comptait l'impôt successoral deux fois — à corriger avant toute résurrection depuis git).
