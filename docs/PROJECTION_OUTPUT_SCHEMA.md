...
| `dayMovementsTotal` | `number` | **PASSÉ UNIQUEMENT.** Nombre TOTAL de mouvements du jour. ⚠️ **Il n'y a PLUS de plafond d'affichage** (`[FUTUR-MOUVEMENTS-TOUS]`, 2026-09-18 : le plafond valait 6, gardait les six PREMIERS rencontrés et non les plus gros — mesuré, 9 journées réelles le dépassaient, la pire à 18). Ce champ survit parce qu'il compte AUSSI les transactions **sans description**, qui n'entrent dans aucune liste affichée : l'écart `dayMovementsTotal − dayMovements.length` ne parle donc plus que d'elles, et c'est ce que « +N autres » affiche. Source : `dailyPastLedger.movementsTotal`. |
