...
> à la fin.
>
> ## 🟢 Session 2026-07-24 (suite) — backlog « tout » VIDÉ : …+ AITOOLS-PROMPT-CACHE + A11Y-DASH-SRONLY (PR #505→#510 MERGÉES ; PR #511 DRAFT = dernier item)
> **✅ `[A11Y-DASH-SRONLY]`** (PR #511, DRAFT) : convention globale d'accessibilité pour l'état vide « — ». Helper pur `components/ui/emptyAware` (tiret `aria-hidden` + `sr-only` « Pas de donnée ») appliqué au CENTRE (slot `value` de `KPIStat` hors privacy + branche non-privée de `PrivateAmount` → couvre `DualKPIStat`) — pas site-par-site. Miroir de `PrivateAmount`. Tests + discriminant. **C'est le DERNIER item du backlog « tout » de Marc.**
> **🔵 NOUVEAU ticket Marc `[FUTUR-REAL-HISTORY]`** (M-L, backlog) : la courbe Futur doit montrer l'historique RÉEL du patrimoine AVANT aujourd'hui (reconstruit depuis transactions + investissements, ancré au NW présent exact, raccordé à la projection future), mis à jour chaque jour + à chaque upload/changement. Plan-first + architect + financial-integrity. = prochaine grosse feature.
