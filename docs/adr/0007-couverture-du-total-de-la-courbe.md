...
**Statut** : accepté (demande Marc 2026-07-22 : « le total de la courbe est nettement plus bas que ce que j'ai vraiment » ;
...
**Contexte** : une part notable des titres détenus (des tickers européens sans candles
chez les providers gratuits) étaient EXCLUS des totaux de la courbe → le TOTAL affiché contredisait
le patrimoine réel (vérifié Google Finance/courtier). Un TOTAL amputé est un chiffre FAUX affiché
...
3. Queue PÉRIMÉE (> 7 j) → raccord au `currentPrice` sur les 7 derniers jours SI la quote live est fraîche
   (`priceUpdatedAt` < 7 j) — cas « quote OK, candles cassées » (un ETF européen détenu) ; sinon le titre sort de la courbe
