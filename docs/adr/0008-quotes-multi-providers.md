...
**Contexte** : après [HIST-COVERAGE-TOTAL], le TOTAL restait à un niveau nettement inférieur à la valeur réelle du portefeuille et des titres
...
2. **Le symbole de COTATION suit la résolution** : `priceRefresh` quote `historySymbol || symbol`
   (un ticker résolu `ABC` → `ABC.PA` sert aux quotes aussi, patch keyé par le symbole de l'actif).
