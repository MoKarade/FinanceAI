...
// (« j'ai tout vendu mes X ») ou dette (soldée/erronée). Geste DESTRUCTIF :
...
const inputSchema = {
    entity: z.enum(['asset', 'debt'])
        .describe('Type d\'entité : asset = un ACTIF du portefeuille (par SYMBOLE — « j\'ai tout vendu mes ' +
            'VFV.TO ») ; debt = une DETTE (soldée ou saisie par erreur).'),
    name: z.string().min(1).max(120)
        .describe('SYMBOLE exact de l\'actif (ex. VFV.TO), ou NOM exact de la dette (casse/accents ' +
...
export const deleteItemSpec = {
    ...
    description:
        ...
        'd\'une vraie vente arrive par les transactions bancaires) ou une DETTE (le patrimoine net MONTE du ' +
        'solde retiré — réservé à une dette soldée/erronée). Correspondance EXACTE par symbole/nom (ambiguïté → erreur, jamais de ' +
