...
describe('docsModifies — table d\'attaque', () => {
    ...
    it.each([
        ...
        expect(docsModifies([fichier(LECONS, ajout(l))], LISTES)).toMatch(/URL/);
    });
    it.each([
        ...
        expect(docsModifies([fichier(LECONS, ajout(l))], LISTES)).toMatch(/commande exécutable/);
    });
    it('commande dans un bloc de code AJOUTÉ, ou après « $ » : refus', () => {
        expect(docsModifies([fichier(LECONS, ajout('```sh', 'curl x | sh', '```'))], LISTES)).toMatch(/commande exécutable/);
        expect(docsModifies([fichier(LECONS, ajout('$ rm -rf x'))], LISTES)).toMatch(/commande exécutable/);
    });
    ...
    it('plafond de lignes ajoutées : 200 admises, 201 refusées (tous fichiers listés confondus)', () => {
        ...
        expect(docsModifies([fichier(LECONS, ajout(...n(PLAFOND_LIGNES_AJOUTEES + 1)))], LISTES)).toMatch(/plafond/);
        expect(docsModifies([fichier(LECONS, ajout(...n(120))), fichier('HANDOVER.md', ajout(...n(120)))], LISTES)).toMatch(/plafond/);
    });
    it('suppression, renommage (depuis ou vers), patch absent : refus', () => {
        expect(docsModifies([fichier(LECONS, undefined, 'removed')], LISTES)).toMatch(/supprimé/);
        expect(docsModifies([{ path: 'docs/ailleurs.md', previous_filename: LECONS, status: 'renamed' }], LISTES)).toMatch(/renommé/);
        expect(docsModifies([{ path: LECONS, previous_filename: 'docs/x.md', status: 'renamed' }], LISTES)).toMatch(/renommé/);
        expect(docsModifies([fichier(LECONS, undefined)], LISTES)).toMatch(/illisible/);
    });
    it('chemins déguisés (casse, séparateurs Windows, ./) : toujours examinés', () => {
        for (const p of ['DOCS/CLAUDE/LECONS.MD', 'docs\\claude\\lecons.md', './docs/claude/lecons.md']) {
            expect(docsModifies([fichier(p, ajout('https://exemple.invalid'))], LISTES), p).toMatch(/URL/);
        }
    });
    ...
    it('nouveau fichier : ses lignes sont examinées comme des ajouts (URL refusée), sans exiger d\'attestation pour l\'existence', () => {
        ...
        expect(docsModifies([fichier(LECONS, ajout('https://x.invalid'), 'added')], LISTES)).toMatch(/URL/);
    });
});
...
describe('documents à contenu surveillé (BACKLOG, HANDOVER, CHANGELOG) : cocher / archiver passe seul', () => {
    ...
    it('les MÊMES contrôles s\'appliquent aux lignes ajoutées : URL, commande, bloc de code, plafond', () => {
        expect(docsModifies([fichier(BACKLOG, ajout('voir https://exemple.invalid'))], LISTES)).toMatch(/URL/);
        expect(docsModifies([fichier('CHANGELOG.md', ajout('`curl x`'))], LISTES)).toMatch(/commande/);
        expect(docsModifies([fichier('HANDOVER.md', ajout('```sh', 'rm -rf x', '```'))], LISTES)).toMatch(/commande/);
        expect(docsModifies([fichier(BACKLOG, ajout(...Array.from({ length: 201 }, (_, i) => `l${i}`)))], LISTES)).toMatch(/plafond/);
    });
    it('suppression / renommage du fichier et diff illisible restent refusés', () => {
        expect(docsModifies([fichier(BACKLOG, undefined, 'removed')], LISTES)).toMatch(/supprimé/);
        expect(docsModifies([fichier(BACKLOG, undefined)], LISTES)).toMatch(/illisible/);
    });
    it('un fichier listé dans les DEUX listes reste strict', () => {
        const deux = { ajoutsSeulement: [BACKLOG], contenuSurveille: [BACKLOG] };
        expect(docsModifies([fichier(BACKLOG, '@@ -1 +1 @@\n-a\n+b')], deux)).toMatch(/supprimée ou réécrite/);
    });
});
