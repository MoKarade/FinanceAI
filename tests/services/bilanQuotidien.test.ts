...
import { curveFieldsDuComposant } from '../helpers/futureSource';
...
describe('[JOUR-BILAN-ROMPU-SOUS-HYPOTHEQUE] le patrimoine au jour EST son bilan', () => {
    ...
    // ⚠️ [DETTE-LEVIER-EXPLICITE 2026-09-21] L'extracteur a déménagé vers
    // `tests/helpers/futureSource.ts` : il vivait ici ET dans `futureProjection.curveFields.test.ts`
    // — et cette seconde copie lisait encore la source BRUTE, donc elle portait toujours la bombe
    // décrite juste au-dessus. Consolider les deux règle celle qui n'avait jamais été corrigée
    ...
    it('[CURVE-FIELDS-DETTE-MANQUANTE] la recomposition s’exécute avec le set de champs de la VRAIE courbe', () => {
        ...
        // Anti-vacuité du scan : portée par `curveFieldsDuComposant` (set non vide), plus un
        // témoin ici — si la regex ne matchait plus, tout ce qui suit serait trivialement satisfait.
