...
export const FISCAL_CONST_INVENTORY: readonly InventoryEntry[] = [
    ...
    { file: 'services/projection/childrenReee.ts', value: '98000', family: 'fiscal',
      reason: '⚠️ VALEUR FAUSSE, PAS SEULEMENT NON SOURCÉE. `rqapCap = 98000 * expenseMultiplier` est le plafond de revenu assurable RQAP figé à sa valeur 2025, alors que la source unique `utils/tax.ts` porte `RQAP_MAX_INCOME = 103000` (§2). Ticket dédié `[RQAP-CAP-98K]` — à remplacer par un import, pas à inventorier durablement.' },
    { file: 'services/projection/childrenReee.ts', value: '0.55', family: 'fiscal',
      reason: 'Taux de remplacement du revenu appliqué au congé parental . Le régime de BASE du RQAP verse 70 % puis 55 % selon la phase ; un 0,55 plat sous-estime le début du congé. Ni le taux ni le choix de la phase ne sont dans FISCAL_REFERENCE. Ticket `[RQAP-CAP-98K]`.' },
    ...
    { file: 'services/projection/realEstateMonth.ts', value: '0.5', family: 'design',
      reason: 'Moitié des frais de copropriété retenue dans le calcul d’un but immobilier (but immobilier). Hypothèse de modèle sur la part récupérable/évitable, aucune règle derrière.' },
