...
export const FISCAL_CONST_INVENTORY: readonly InventoryEntry[] = [
    ...
    { file: 'services/projection/realEstateMonth.ts', value: '0.65', family: 'fiscal',
      reason: '[×2] Ratio prêt/valeur au-delà duquel le modèle déclenche un appel de marge (test du seuil, puis calcul du surplus). ⚠️⚠️ RAISON RÉÉCRITE le 2026-09-18 : elle citait « le §8 dit LTV 65 %, plafond B-20 de la portion réavançable » pour justifier le classement `fiscal` — or `[SMITH-MARGE-SANS-DETTE]` a RETIRÉ cette phrase de §8 le jour même, parce qu’elle accolait la formule du CODE (`marge + hypothèque ≤ 65 %`) à une règle qui, elle, borne la seule portion réavançable. La raison s’appuyait donc sur une citation morte, et commettait exactement ce qu’elle reprochait : contredire la section qu’elle invoque. Ce que §8 dit MAINTENANT : c’est une hypothèse de MODÈLE, l’écart avec B-20 est nommé, et la décision est routée (`[SMITH-LTV-SEUIL-65]`). ⚠️ `family: fiscal` est CONSERVÉ à dessein — le rétrograder sortirait la valeur du tri fiscal pendant que la décision est ouverte, et une protection ne se retire pas pour cause de justification réécrite ; la question « fiscal ou design ? » part avec le ticket. ⚠️ Une garde d’anti-fantôme vérifie que le littéral existe encore, jamais qu’une raison dit vrai : c’est un panel qui l’a vu.' },
    ...
    { file: 'services/projection/helpers.ts', value: '0.1879', family: 'fiscal',
      reason: '`RRIF_RATES[94]` — facteur de retrait MINIMUM du FERR à 94 ans (18,79 %), barème ARC ancré FISCAL_REFERENCE §7. Statutaire : il pilote un retrait FORCÉ, donc de l’impôt. ⚠️ Historique : cette entrée portait 0.2000 (le plateau, appliqué un an trop tôt) et était la SEULE de la table marquée CONTESTÉE ; corrigée le 2026-09-05 (`[FISC-RRIF-94-FACTOR]`, lot 168) sur source RELAYÉE — quatre tables 2026 concordantes, le règlement 7308(4) restant illisible depuis le conteneur. Mesuré : +1 367 $ à +4 778 $ de patrimoine final selon le REER encore détenu à 94 ans.' },
    ...
    { file: 'services/projection/retirementIncome.ts', value: '0.5', family: 'fiscal',
      reason: '[≠2] DEUX LIGNES depuis le lot 105, TROIS littéraux — `survivorRrqFactor` en porte deux sur la même ligne. `survivorRrqFactor` et `survivorPsvFactor` — facteurs de rente au SURVIVANT : la PSV du défunt cesse (facteur 0,5 sur un couple) et la RRQ est recalculée via `rrqSurvivorPct`. Règles de Service Canada / Retraite Québec, §6. La quatrième ligne d’avant portait la prime d’indexation de 0,5 pp du MGA : elle a QUITTÉ ce fichier pour `helpers.ts` (`MGA_EXCES_SUR_INFLATION_PP`), source unique du patron — c’est cette garde qui a exigé de re-compter.' },
    { file: 'services/projection/helpers.ts', value: '0.5', family: 'design',
      reason: '[≠2] DEUX SENS sans rapport. (1) `MGA_EXCES_SUR_INFLATION_PP` — prime d’indexation d’un demi-point au-dessus de l’inflation, SOURCE UNIQUE du patron MGA depuis le lot 105 (`[MGA-PATRON-5-COPIES]`). Elle remplace cinq littéraux identiques (retirementIncome ×2, activeIncome, childrenReee, taxJanuary) et couvre le MGA de la RRQ, le plafond RQAP, le maximum assurable de l’AE et l’extrapolation du plafond REER. Les PLAFONDS sont sourcés (ARC / Retraite Québec) ; la VITESSE à laquelle on les prolonge est une hypothèse de MODÈLE, documentée §7 avec son écart mesuré contre l’indexation observée — ne JAMAIS la « sourcer ». ⚠️ Ce qui est partagé est la vitesse, pas l’ANCRE : chaque appelant passe SON nombre d’années. (2) `round2` (lot 131) : la DEMI-FRONTIÈRE d’arrondi (`|frac| − 0.5`) — arithmétique pure, rien de fiscal.' },
    { file: 'services/projection/helpers.ts', value: '1e-12', family: 'structural',
      reason: '`round2` (lot 131) : plancher ABSOLU de la fenêtre d’incertitude autour des demi-frontières (le terme relatif `4·|scaled|·EPSILON` s’annule près de zéro). Epsilon d’algorithme d’arrondi, pas un paramètre fiscal — la PARITÉ avec `toFixed` est prouvée par le fuzz de `projection.round2.test.ts`, c’est lui qui fait autorité, pas ce chiffre.' },
